...
function setHeatmap(){
	...
	var trequest = "../provider/deliver.php?color=red&zoom="+map.getZoom()+"&min_y="+min_y+"&max_y="+max_y+"&min_x="+min_x+"&max_x="+max_x+"&vis=square&vis_typ=value_max&time="+time+"&ref_id="+(quake_id+1);
    ...
    eventSource.onmessage = function(event){
		...
		for(var i = 0; i<frames[0].length; i++){
			...
			if(frames[0][i].geometry.coordinates[0].length >= 1){
				geo = L.Proj.geoJson(frames[0][i], { style: function(feature){ return feature.properties && feature.properties.style; }}).on('dblclick', function(event){map.panTo(event.latlng);map.zoomIn();});
				geo.addTo(layer);
			}
		}
