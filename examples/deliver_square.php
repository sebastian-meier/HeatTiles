...
if ($result) {
	while ($row = mysql_fetch_array($result)) {
		...
		array_push($tarray, array($x*$step_size[$step_max-$z0]-$max, $y*$step_size[$step_max-$z0]-$max));
		array_push($tarray, array($x*$step_size[$step_max-$z0]-$max+$step_size[$step_max-$z0], $y*$step_size[$step_max-$z0]-$max));
		array_push($tarray, array($x*$step_size[$step_max-$z0]-$max+$step_size[$step_max-$z0], $y*$step_size[$step_max-$z0]-$max+$step_size[19-$z0]));
		array_push($tarray, array($x*$step_size[$step_max-$z0]-$max, $y*$step_size[$step_max-$z0]-$max+$step_size[$step_max-$z0]));
...
?>{"type": "Feature", "properties":{"style": {"stroke": "false", "weight":"0", "color":"#000", "fillColor":"#ff0000", "opacity":"0", "fillOpacity":"0.<?php echo $key+1; ?>"}}, "geometry": {"type": "MultiPolygon","coordinates": <?php echo json_encode(array($step)); ?>},"crs": {"type": "name","properties": {"name": "urn:ogc:def:crs:SR-ORG::7483"}}}<?php
}
echo ']';?>