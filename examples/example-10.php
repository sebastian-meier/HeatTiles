...
<html>
  ...
  <body>
    ...
    <script type="text/javascript" src="js/webgl/shCore.js"></script>
    <script type="text/javascript" src="js/webgl/shBrushJScript.js"></script>
    <script type="text/javascript" src="js/webgl/webgl-heatmap.js"></script>
    <script type="text/javascript" src="js/webgl/webgl-heatmap-leaflet.js"></script>
    <script>
    	...
    	$(document).ready(function() {
			...
			function setHeatmap(){
				...
				var trequest = "deliver_heathex.php?zoom="+map.getZoom()+"&min_y="+min_y+"&max_y="+max_y+"&min_x="+min_x+"&max_x="+max_x;
				if(trequest!=request){
					request = trequest;
					$.getJSON(request, function( data ) {
						...
						var zoom = data.zoom;
						if(zoom<14){
							step = Math.pow(2, (14-zoom));
						}else if(zoom>14){
							step = 1 / Math.pow(2, (zoom-14));
						}

						data.data.push([180,90,data.max]);
						var heatmap = new L.TileLayer.WebGLHeatMap({size: step*300, alphaRange: 1, opacity: 0.7});
