    ...
    <script>
    	...
    	$(document).ready(function() {
			...
			function setHeatmap(){
				...
				var trequest = "deliver_circle.php?zoom="+map.getZoom()+"&min_y="+min_y+"&max_y="+max_y+"&min_x="+min_x+"&max_x="+max_x;
				if(trequest!=request){
					request = trequest;
					$.getJSON(request, function( data ) {
						...
						for(var i = 0; i<data.length; i++){
							...
							}).on('dblclick', function(event){map.panTo(event.latlng);map.zoomIn();});
							geo.addTo(layer);
						}
					});
				}
